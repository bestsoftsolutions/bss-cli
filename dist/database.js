import c from'oracledb';import {debug}from'./logger.js';import {loadConfig}from'./configLoader.js';async function S(){try{const r=loadConfig(),t={user:r.axpert.db?.user??"",password:r.axpert.db?.password??"",connectString:r.axpert.db?.connectString??""};return await c.getConnection(t)}catch(r){console.error("Error connecting to Oracle:",r),process.exit(1);}}async function N(r,t){const a=`
        SELECT 
            COLUMN_NAME,
            DATA_TYPE,
            DATA_LENGTH,
    ...
    `;try{return (await r.execute(a,{tableName:t.toUpperCase()})).rows?.map(l=>l[0])||[]}catch(e){return console.warn("Warning: Could not retrieve primary keys:",e),[]}}async function f(r,t){const a=`
