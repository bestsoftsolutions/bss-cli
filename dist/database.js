import i from'oracledb';import {debug}from'./logger.js';import {loadConfig}from'./configLoader.js';async function T(){try{const t=loadConfig(),a={user:t.axpert.db?.user??"",password:t.axpert.db?.password??"",connectString:t.axpert.db?.connectString??""};return await i.getConnection(a)}catch(t){console.error("Error connecting to Oracle:",t),process.exit(1);}}async function d(t,a){const e=`
    ...
    `;try{return (await t.execute(e,{tableName:a.toUpperCase()})).rows?.map(n=>n[0])||[]}catch(r){return console.warn("Warning: Could not retrieve primary keys:",r),[]}}async function C(t,a){const e=`
...
Tables found for structure '${a}':`);for(const E of r.rows){const u=E[0];E[1]==="T"?(s.push(u),console.log(`   \u{1F4C4} DETAIL: ${u}`)):(n.push(u),console.log(`   \u{1F4CB} MASTER: ${u}`));}return {master:n,details:s}}catch(r){console.error("\u274C Error querying database:",r),process.exit(1);}}async function _(t,a){const e=`
...
`;)n+=t[e++];a.push(n);continue}if(r==="/"&&t[e+1]==="*"){let n="/*";for(e+=2;e<t.length&&!(t[e]==="*"&&t[e+1]==="/");)n+=t[e++];e<t.length&&(n+="*/",e+=2),a.push(n);continue}a.push(r.toLowerCase()),e++;}return a.join("")}const w=t=>{const a=/'(?:[^']|\\')*'/g,e=t.replace(a,"NO PARAM"),r=/\?|\:\w+/g,s=e.match(r)||[];return Array.from(new Set(s.map(E=>E.replace(":","").replace("@","").toLowerCase())))};export{h as convertSqlToLowercase,w as extractParams,D as getAxpertStructure,I as getFieldsForFrontendSchema,y as getFieldsForParameters,_ as getFieldsWithSql,T as getOracleConnection,N as getPrimaryKeys,b as getStatementParameters,d as getTableSchema,C as getTablesByStructure};